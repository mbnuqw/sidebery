...
void async function () {
  ...
  if (!reqBtnEl) return; // Get permissions

  const origins = [];
  const permissions = [];
  let perm = reqBtnEl.getAttribute('data-perm');
  if (perm === 'all_urls') origins.push('<all_urls>');else if (perm) permissions.push(perm);
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "38365" + '/');
