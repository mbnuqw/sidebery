...
\`\`\`
Доступные параметры:
- \`url:{RegExp}\` и/или \`title:{RegExp}\`: Регулярное выражение для сопоставления вкладки и опционального получения значения бейджа через именованную группу: \`(?<v>...)\`. Если ваше \`{RegExp}\` содержит точку с запятой с пробелом \`; \`, экранируйте пробел обратным слэшем: \`;\\ \`. Значение по умолчанию \`{RegExp}\` — пустая строка. Бейдж без значения игнорируется для выгруженных или активных вкладок.
- \`bg:{Color}\` и/или \`fg:{Color}\`: Цвет фона или текста бейджа.
- \`urgent\`: Делает бейдж срочным и распространяет этот статус на родительские элементы (свернутые вкладки, неактивные панели).
