...
export interface TabMoveRulesPopup {
  panelId: ID
  rules: T.TabToPanelMoveRuleConfig[]
}
...
export function reactivate(r: T.Reactivator<PopupsReactiveState>) {
  reactive = r(reactive)
}

export function confirm(msg: string, type?: E.ConfirmationType): Promise<boolean> {
  return new Promise(res => {
    reactive.confirm = {
      type: type ?? E.ConfirmationType.Unknown,
      msg,
      ok: () => {
...
export function ask(conf: T.DialogConfig): Promise<string | null> {
...
export function openPanelPopup(conf: Partial<T.PanelConfig>, index?: number): Promise<ID | null> {
  if (conf.type === undefined) conf.type = E.PanelType.tabs
...
export function openNewTabShortcutsPopup(panel?: T.PanelConfig): void {
...
export function openSiteConfigPopup(tab: T.Tab): void {
