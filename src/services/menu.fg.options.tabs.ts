...
export const tabsMenuOptions: Record<string, () => MenuOption | MenuOption[] | undefined> = {
  ...
  moveToNewWin: () => {
    return {
      ...
      onClick: () => {
        ...
        Tabs.move(items, {}, { windowId: D.NEWID, incognito: Windows.incognito })
      },
    }
  },
  ...
  reopenInWin: () => {
    ...
    const wins = Windows.otherWindows.filter(w => w.incognito !== Windows.incognito)
    const winLen = wins.length
    const containerId = Windows.incognito ? D.DEFAULT_CONTAINER_ID : D.PRIVATE_CONTAINER_ID
  ...
  reopenInCtr: () => {
    ...
    if (firstTab.cookieStoreId !== D.CONTAINER_ID) {
      opts.push({
        label: translate('menu.tab.reopen_in_default_container'),
        icon: 'icon_ff',
        badge: 'icon_reopen',
        onClick: () => Tabs.reopenInContainer(Selection.ids(), D.CONTAINER_ID),
      })
    }
  ...
  openTabsInCtr: () => {
    ...
    if (firstTab.cookieStoreId !== D.CONTAINER_ID) {
      opts.push({
        ...
        icon: 'icon_ff',
        onClick: () => Tabs.openInContainer(Selection.ids(), D.CONTAINER_ID),
      })
    }
  ...
  bind_url: () => {
    ...
    return {
      ...
      icon: firstTab.anchorUrl ? 'icon_home' : 'icon_undo',
  ...
  colorizeTab: () => {
    ...
    for (const color of D.COLOR_OPTS) {
