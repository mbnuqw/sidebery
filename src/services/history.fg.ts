...
export const setAllLoadedState = (r: boolean) => (ready = r)
export let loadingMore = false

export let panelScrollEl: HTMLElement | null = null
export const setPanelScrollEl = (el: HTMLElement | null) => (panelScrollEl = el)
export let subPanelScrollEl: HTMLElement | null = null
export const setSubPanelScrollEl = (el: HTMLElement | null) => (subPanelScrollEl = el)
...
export function createVisit(
  nItem: T.NativeHistoryItem,
  nVisit?: T.NativeVisit,
  itemDomain?: string,
...
export async function load(): Promise<void> {
  ...
  const loadedVisits = await normalizeHistory(result, true, lastItemVisitTime)

  if (!loadedVisits.length) await loadMore()
  else visits = loadedVisits
...
export function recalcDays() {
  const days: T.HistoryDay[] = []
  const dayStart = Utils.getDayStartMS()
  let day: T.HistoryDay | undefined
  let lastDayId = ''
  let prevVisit: T.Visit | undefined
...
let cachedVisits: Record<string, T.NativeVisit[]> = {}
...
export async function loadMore(): Promise<void> {
  ...
  if (result.length) {
    ...
    if (newVisits.length) {
      ...
      reactive.days = History.recalcDays()
      loadingMore = false
      return
    }
  }
...
function getLastVisitTime(list?: T.Visit[]): number {
...
function onVisit(item: T.NativeHistoryItem): void {
